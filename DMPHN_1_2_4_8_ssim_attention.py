...
def main():
    ...
    attention_lv4 = attention_blocks[0](128)
    ...
    attention_lv4.apply(weight_init).cuda(GPU)
    ...
    attention_lv4_optim = torch.optim.Adam(attention_lv4.parameters(),lr=LEARNING_RATE)
    attention_lv4_scheduler = StepLR(attention_lv4_optim,step_size=1000,gamma=0.1)
    ...
    for epoch in range(args.start_epoch, EPOCHS):
        ...
        for iteration, images in enumerate(train_dataloader):            
            ...
            feature_lv4_5 = attention_lv4(feature_lv4_4)
            feature_lv4_6 = attention_lv4(feature_lv4_5)
            feature_lv4_7 = attention_lv4(feature_lv4_6)
            feature_lv4_8 = attention_lv4(feature_lv4_7)
            ...
            residual_lv4_top_left = torch.clamp(residual_lv4_top_left, min=-0.5, max=0.5)
            residual_lv4_top_right = torch.clamp(residual_lv4_top_right, min=-0.5, max=0.5)
            residual_lv4_bot_left = torch.clamp(residual_lv4_bot_left, min=-0.5, max=0.5)
            residual_lv4_bot_right = torch.clamp(residual_lv4_bot_right, min=-0.5, max=0.5)
        ...
        if (epoch)%100==0:
            ...
            test_dataset = MyDataset(
                blur_image_files = './datas/myData/test_blur_file.txt.txt',
        ...
        torch.save(attention_lv4.state_dict(),str('./checkpoints/' + METHOD + "/attention_lv4.pkl")) 
